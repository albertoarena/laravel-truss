...
- **A user-facing change updates `CHANGELOG.md` under `## [Unreleased]`, in the same pull request.** Not at release time: an entry written then is written from a diff by somebody reconstructing intent. The same test as the docs rule below (a new or changed command, config key, payload shape, route, or visible dashboard behaviour). A bug introduced and fixed within the same unreleased cycle gets no `Fixed` line, because that line tells readers the *previous release* was broken. Full detail, including the release steps, in `.claude/rules/release.md`.
