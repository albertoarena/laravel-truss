...
**Scope:** Phase 1 ships the engine and thirteen rules (integrity, index, type), console and JSON output, presets, and exit codes. Suppression files, CI formatters, more rules, a dashboard panel, and fix stubs are later phases. No new runtime dependency, and no network call anywhere.
