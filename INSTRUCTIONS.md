...
- 2026-07-23 — Phase 5: HTTP layer. Fixed `viewTruss` gate (package default allows `local` only, nullable-user callback so guests are evaluated, defined only if the host has not; host overrides win). `Authorize` middleware: 404 when `truss.enabled` is off (routes act as if absent), 403 when the gate denies. Routes registered under `truss.route_prefix` (`truss.index`, `truss.api.schema`). `IndexController` renders the `truss::index` Blade shell (endpoint + config in data-attributes, no schema inlined). `SchemaApiController`: resolves the requested connection (default when absent), 404s connections not in `managedConnections()`, filters `excluded_tables` (global + per-connection) server-side at serve time from the full cached snapshot, carries the `fallback` flag. Tested: gate blocks (403), disabled hides (404), JSON envelope shape, excluded tables never on the wire (non-negotiable), per-connection exclusions, unmanaged connection not visualizable.
